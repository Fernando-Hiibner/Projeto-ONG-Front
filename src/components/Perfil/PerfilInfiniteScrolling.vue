...
export default {
  ...
  computed: {
    url() {
      return "loadUserPub/"+this.page.toString();
    },
  },
  created() {
    this.fetchData();
  ...
  methods: {
    async fetchData() {
      const response = await this.axios.post(this.url, {"email": this.$store.getUser});
    ...
    infiniteScrolling() {
      setTimeout(() => {
        this.page++;
        this.axios
          .post(this.url, {"email": this.$store.getUser})
          .then((response) => {
